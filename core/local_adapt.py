...
def local_adaptation(clientObj):
    ...
    clientObj.local_adapt_train(adapt_trainloader)
    # reset the requires_grad
    clientObj.freeze_except_adapter()
    return clientObj
...
def fedts(weights, clientObjs, server):
    ...
    for id in range(len(clientObjs)):
        clientObjs[id].model.base.global_adapter.load_state_dict(server_global_adapter.state_dict())
