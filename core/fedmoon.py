...
def run(args):
    ...
    with open(f'./costs/fedmoon/{args.image_encoder_name}_{args.dataset}_sub{args.subset_size}.json', 'a+') as f:
            json.dump({"Communication_cost": f"{(communication_cost + communication_cost_head)/ (1024 ** 2):.4f}"}, f)
            f.write('\n')
...
if __name__ == "__main__":
    ...
    os.makedirs(f'./costs/fedmoon/', exist_ok=True)
    with open(f'./costs/fedmoon/{args.image_encoder_name}_{args.dataset}_sub{args.subset_size}.json', 'w+') as f:
