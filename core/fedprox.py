...
def run(args):
    ...
    for ist in range(len(clients_ids)):
        ...
        client = ClientProx(args, ist, cd.train_dataset, cd.test_dataset, cd.val_dataset, cd.train_loader, cd.test_loader,
    ...
    for r in range(args.global_rounds):
        ...
        if (r % args.eval_interval == 0 or r == args.global_rounds - 1) or early_stop or counter == 0:
            ...
            with open(f'./results/fedprox/{args.image_encoder_name}_{args.dataset}_sub{args.subset_size}_split{args.split_num}.json', 'a+') as f:
...
if __name__ == "__main__":
    ...
    with open(f'./results/fedprox/{args.image_encoder_name}_{args.dataset}_sub{args.subset_size}_split{args.split_num}.json', 'w+') as f:
