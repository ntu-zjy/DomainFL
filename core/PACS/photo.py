...
class Photo:
    def __init__(self, train_preprocess, val_preprocess, location=os.path.expanduser('~/data/PACS'), batch_size=128, num_workers=0):
        self.train_dataset = BasePACSDataset(location, domain='photo', split='train', transform=train_preprocess)
        self.test_dataset = BasePACSDataset(location, domain='photo', split='test', transform=val_preprocess)
