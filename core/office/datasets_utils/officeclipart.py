...
class OfficeClipart:
    def __init__(self, train_preprocess, val_preprocess, location=os.path.expanduser('~/data'), batch_size=128, num_workers=0):
        ...
        self.train_dataset = BaseOfficeHomeDataset(location, domain='OfficeClipart', split='train', transform=train_preprocess)
        self.test_dataset = BaseOfficeHomeDataset(location, domain='OfficeClipart', split='test', transform=val_preprocess)
