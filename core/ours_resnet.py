...
np.random.seed(1)
torch.manual_seed(1)
torch.cuda.manual_seed_all(1) if torch.cuda.is_available() else None

def are_models_equal(model1, model2, rtol=1e-05, atol=1e-08):
    """
    检测两个模型的参数是否一致。
    ...
    返回:
        bool: 如果两个模型的所有参数一致，返回 True，否则返回 False。
    """
    # 检查模型结构是否一致
...
def run(args):
    ...
    print("equal:", are_models_equal(clients[0].model.head, clients[1].model.head))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-ien','--image_encoder_name', type=str, default='RN50', help='Image encoder name')
