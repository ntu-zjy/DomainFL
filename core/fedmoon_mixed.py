...
def run(args):
    ...
    for r in range(args.global_rounds):
        ...
        if (r % args.eval_interval == 0 or r == args.global_rounds - 1) or early_stop or counter == 0:
            ...
            with open(f'./results/fedmoon_mixed/{args.image_encoder_name}_{args.dataset}_sub{args.subset_size}_mixed{args.mixed_ratio}.json', 'a+') as f:
...
if __name__ == "__main__":
    ...
    os.makedirs(f'./results/fedmoon_mixed/', exist_ok=True)
    with open(f'./results/fedmoon_mixed/{args.image_encoder_name}_{args.dataset}_sub{args.subset_size}_mixed{args.mixed_ratio}.json', 'w+') as f:
