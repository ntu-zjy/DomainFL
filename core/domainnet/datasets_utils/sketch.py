...
class Sketch:
    def __init__(self, train_preprocess, val_preprocess, location=os.path.expanduser('~/data/DomainNet'), batch_size=128, num_workers=0):
        self.train_dataset = BaseDomainNetDataset(location, domain='sketch', split='train', transform=train_preprocess)
        self.test_dataset = BaseDomainNetDataset(location, domain='sketch', split='test', transform=val_preprocess)
