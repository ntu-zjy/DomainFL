...
class synthetic:
    def __init__(self, train_preprocess, val_preprocess, location=os.path.expanduser('~/data/Adaptiope'), batch_size=128, num_workers=0):
        self.train_dataset = BaseAdaptiopeDataset(location, domain='synthetic', split='train', transform=train_preprocess)
        self.test_dataset = BaseAdaptiopeDataset(location, domain='synthetic', split='test', transform=val_preprocess)
