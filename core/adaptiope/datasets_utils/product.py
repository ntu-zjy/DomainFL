...
import torch
import numpy as np
from .AdaptiopeBase import BaseAdaptiopeDataset

class product:
    def __init__(self, train_preprocess, val_preprocess, location=os.path.expanduser('~/data/Adaptiope'), batch_size=128, num_workers=0):
        self.train_dataset = BaseAdaptiopeDataset(location, domain='product_images', split='train', transform=train_preprocess)
        self.test_dataset = BaseAdaptiopeDataset(location, domain='product_images', split='test', transform=val_preprocess)
