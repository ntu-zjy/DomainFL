...
class BaseDomainNetDataset(Dataset):
    def __init__(self, root_dir, domain, split='train', transform=None, class_sample_dict=None):
        ...
        self.class_to_idx = \
        self.img_labels[['class_name', 'label']].drop_duplicates().set_index('class_name').to_dict()['label']

        # 控制特定类别的数据个数
        if class_sample_dict is not None:
            self.img_labels = self.img_labels.groupby('label').apply(
                lambda group: self._limit_class_samples(group, class_sample_dict)
            ).reset_index(drop=True)

    def _limit_class_samples(self, group, class_sample_dict):
        label = group['label'].iloc[0]
        if label in class_sample_dict:
            return group.sample(n=min(len(group), class_sample_dict[label]), random_state=42)
        return group
    ...
    def download_and_extract_data(self):
        ...
        download_and_extract_archive(url=self.data_zip_url, download_root=f"{self.root_dir}",
                                     filename=f"{self.domain}.zip", md5=None)
