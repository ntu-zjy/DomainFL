...
def run(args):
    ...
    for r in range(args.global_rounds):
        ...
        if (r % args.eval_interval == 0 or r == args.global_rounds - 1) and r!=0:
            ...
            with open(f'./results/fedavg_la_DG/{args.image_encoder_name}_{args.dataset}_sub{args.subset_size}.json', 'a+') as f:
...
if __name__ == "__main__":
    ...
    os.makedirs(f'./results/fedavg_la_DG/', exist_ok=True)
    with open(f'./results/fedavg_la_DG/{args.image_encoder_name}_{args.dataset}_sub{args.subset_size}.json', 'w+') as f:
